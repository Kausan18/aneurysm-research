...
FIX: preprocess_features() now returns AR_n and SR_n. All calls to
simulate_and_extract() now pass AR_n=AR_n, SR_n=SR_n so that
r0 = 0.3*SR_n + 0.7*AR_n per Paper Eq. 8. Previously all Phase 2 calls
were missing these arguments, so they fell back to the wrong S_REF anchor.

Also set USE_CALIBRATION = True in models.py before running this file.
...
# Configurable flags
RUN_SURROGATE_REFINEMENT = True
RUN_OPTUNA               = True
RUN_RFECV                = True
PRINT_IMPORTANCES        = True


def run_phase2_diagnostics(best_params, biomarkers_p1, biomarkers_p2,
                           X_baseline, X_ode, X_hybrid,
                           results_ode, results_hybrid, results_baseline):
    ...
    print(f"  {n_improved}/{len(cmp)} biomarkers have higher std under optimised params.")
    ...
    if inf_rows.empty:
        ...
        print(f"  [WARN] inf thresholds still present:\n{inf_rows[['Model', 'Threshold_mean']]}")
...
def main():
    ...
    print("  r0: 0.3*SR_n + 0.7*AR_n  (Paper Eq. 8 -- fixed)")
    print("=" * 65)
    ...
    # Step 5: Optuna ODE coefficient optimisation
    if RUN_OPTUNA:
        print(f"\n[STEP 5] Running Optuna study ({PHASE2_N_TRIALS} trials)...")
        # phase2_optuna._cv_auc_hybrid also calls simulate_and_extract internally.
        # We pass AR_n and SR_n into run_optuna_study so it can forward them.
    ...
    # Step 6: Recompute biomarkers with best_params + correct r0
    print("\n[STEP 6] Recomputing ODE biomarkers with optimised params...")
    biomarkers_p2 = simulate_and_extract(
        L, H, S,
        params=best_params,
        AR_n=AR_n,
        SR_n=SR_n,
    ...
    # Step 9: RFECV feature selection
    if RUN_RFECV:
    ...
    # Step 13: Save all Phase 2 results
    res_baseline.to_csv(
